...
class GatedSAEExperiment(Experiment):
    ...
    regularization_coefficient = torch.tensor([1000.0, 1000.0, 1000.0, 1000.0, 1000.0])
    regularization_l1_coefficients = (2.0, 6.0, 32.0, 48.0, 48.0)  # Targets l0s ~ 10
    regularization_trainable_layers = (0, 1, 2, 3, 4)

    # Sweep range for SAE training on model using normal weights
    sweep_normal_starting_coefficients = (0.5, 1.0, 4.0, 12.0, 16.0)
    sweep_normal_ending_coefficients = (2.0, 6.0, 32.0, 48.0, 48.0)

    # Sweep range for SAE training on model using regularized weights
    sweep_regularized_starting_coefficients = (0.2, 1.0, 4.0, 6.0, 6.0)
    sweep_regularized_ending_coefficients = (1.0, 3.0, 16.0, 24.0, 24.0)
